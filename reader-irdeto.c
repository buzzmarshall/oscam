...
struct irdeto_data
{
	...
	uint16_t acs;
	int32_t acsfox; // A flag for FOX DVB-T
...
static int32_t irdeto_card_init_provider(struct s_reader *reader)
{
	...
	for(buf[0] = i = p = 0; i < reader->nprov; i++)
	{
		...
		else
		{
			...
			rdr_log(reader, "cta_lr=0x%04x", cta_lr);
            if((csystem_data->acsfox == 1) && (cta_lr < 26))
			{
				reader->nprov = i;
				break;
			}
		...
		if(((cta_lr == 26) && ((!(i & 1)) || (cta_res[0] != 0xf))) || (csystem_data->acs57 == 1) || (csystem_data->acsfox == 1))
		{
			reader->prid[i][4] = p++;
...
static int32_t irdeto_card_init(struct s_reader *reader, ATR *newatr)
{
    ...
    if ((reader->caid == 0x0638) || (reader->caid == 0x0642) || (reader->caid == 0x0652))
	{
		csystem_data->acsfox = 1;
		acspadd = 8;
	}
	...
	if(reader->nprov > CS_MAXPROV)
	{
		reader->nprov = CS_MAXPROV;
	}
...
int32_t irdeto_do_ecm(struct s_reader *reader, const ECM_REQUEST *er, struct s_ecm_answer *ea)
{
	...
	uint8_t cw[16];
	memcpy(cw, cta_res + 6 + acspadd, 16);
	if(csystem_data->acsfox == 1)
	{
		cw[3] = cw[0]+cw[1]+cw[2];
		cw[7] = cw[4]+cw[5]+cw[6];
...
static int32_t irdeto_get_emm_filter(struct s_reader *rdr, struct s_csystem_emm_filter **emm_filters, unsigned int *filter_count)
{
	if(*emm_filters == NULL)
	{
		...
		*filter_count = 0;
		...
		unsigned int idx = 0;
		struct irdeto_data *csystem_data = rdr->csystem_data;

		filters[idx].type = EMM_GLOBAL;
		filters[idx].enabled = 1;
		filters[idx].filter[0] = 0x82;
		filters[idx].mask[0] = 0xFF;
		if(csystem_data->acsfox == 0)
		{
		...
		for(i = 0; i < rdr->nprov; i++)
		{
			...
			else
			{
				filters[idx].filter[1] = 0x03;
				filters[idx].mask[1] = 0x03;
				memcpy(&filters[0].filter[2], &rdr->prid[i][1], 3);
				memset(&filters[0].mask[2], 0xFF, 3);
			}
